...
# Small autoencoder: compress features then reconstruct to score how unusual each row is.
class AE(nn.Module):
    # Build encoder/decoder layers sized to input dimension d.
    def __init__(self,d): 
        ...
        # Encode then decode; output is a reconstruction of the input.
    def forward(self,x): z=self.e(x); return self.d(z)
# Train AE on numeric features, compute ae_anomaly from reconstruction error, then save the augmented dataset.
